...
class kl_annealing():
    ...
    def frange_cycle_linear(self, n_iter, start=0.0, stop=1.0,  n_cycle=1, ratio=1):
        ...
        # if self.beta_cycle > 0 and self.current_epoch % self.beta_cycle == 0 :
            iter_with_cycle = self.current_epoch * ratio
...
class VAE_Model(nn.Module):
    ...
    def plot_one_loss(self, num_epoch):
        df = pd.read_csv(self.path_train_loss_wokl) if os.path.exists(self.path_train_loss_wokl) else None
        best_loss = float('inf')
        kl_anneal_type = 'Wkl'
        for loss in df[df.columns[0]]:
            if loss < best_loss:
                best_loss = loss
        print(f'Lowest loss of {kl_anneal_type} : {best_loss}')
